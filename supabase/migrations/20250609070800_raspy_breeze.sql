/*
# Smart Investment Platform Database Schema

This SQL script creates a comprehensive database structure for the Smart Investment Platform
that connects content creators with investors for films and digital media projects.

## Database Structure:
1. Users table - stores both creators and investors
2. Movies table - stores movie/project information
3. Investments table - tracks all investment transactions
4. Investment_history table - maintains investment audit trail
5. User_sessions table - manages user authentication sessions
6. Project_categories table - categorizes different types of projects
7. Payment_transactions table - tracks payment processing

## Features:
- User role management (creator/investor)
...
CREATE TABLE movies (
    ...
    ai_risk_score DECIMAL(3,2) DEFAULT 0.50, -- 0.00 to 1.00 (low to high risk)
...
('creator1', 'creator@test.com', '$2b$10$example_hash_1', 'creator', 'John', 'Director', 'Passionate filmmaker with 10 years of experience in independent cinema.', TRUE),
('creator2', 'sarah.producer@test.com', '$2b$10$example_hash_2', 'creator', 'Sarah', 'Producer', 'Award-winning producer specializing in drama and thriller genres.', TRUE),
('investor1', 'investor@test.com', '$2b$10$example_hash_3', 'investor', 'Michael', 'Johnson', 'Angel investor focused on entertainment and media projects.', TRUE),
('investor2', 'jane.investor@test.com', '$2b$10$example_hash_4', 'investor', 'Jane', 'Smith', 'Portfolio manager with expertise in creative industry investments.', TRUE),
('creator3', 'alex.filmmaker@test.com', '$2b$10$example_hash_5', 'creator', 'Alex', 'Turner', 'Independent filmmaker and screenwriter.', TRUE);

-- Insert sample movies/projects
INSERT INTO movies (title, description, poster_url, director, producer, singer, hero, heroine, category_id, creator_id, total_amount, invested_amount, stock_price, funding_deadline, ai_risk_score, ai_fraud_check) VALUES
('Epic Adventure', 'A thrilling adventure story set in ancient times with stunning visuals and compelling characters.', 'https://images.pexels.com/photos/436413/pexels-photo-436413.jpeg', 'John Director', 'Jane Producer', 'Music Maestro', 'Action Star', 'Lead Actress', 1, 1, 1000000.00, 350000.00, 100.00, '2024-12-31', 0.25, TRUE),
('Romantic Drama', 'A heartwarming love story that explores the complexities of modern relationships.', 'https://images.pexels.com/photos/274937/pexels-photo-274937.jpeg', 'Sarah Director', 'Mike Producer', 'Melody Singer', 'Romantic Lead', 'Drama Queen', 1, 2, 750000.00, 200000.00, 75.00, '2024-11-30', 0.30, TRUE),
('The Last Horizon', 'A sci-fi thriller about humanity\'s last hope for survival in a post-apocalyptic world.', 'https://images.pexels.com/photos/2510428/pexels-photo-2510428.jpeg', 'Sarah Johnson', 'Michael Roberts', 'Electronic Composer', 'Future Hero', 'Sci-Fi Heroine', 1, 1, 2500000.00, 1750000.00, 100.00, '2025-01-15', 0.20, TRUE),
('Whispers of the Heart', 'An intimate character study exploring family dynamics and personal growth.', 'https://images.pexels.com/photos/3062541/pexels-photo-3062541.jpeg', 'Robert Zhang', 'Sophia Davis', 'Classical Composer', 'Character Actor', 'Dramatic Lead', 1, 2, 1200000.00, 850000.00, 80.00, '2024-12-15', 0.35, TRUE),
('Shadows of Truth', 'A gripping mystery thriller that keeps audiences guessing until the final reveal.', 'https://images.pexels.com/photos/2873486/pexels-photo-2873486.jpeg', 'Thomas Miller', 'Jennifer Brown', 'Suspense Composer', 'Detective Hero', 'Mystery Woman', 1, 5, 1800000.00, 450000.00, 70.00, '2025-02-28', 0.40, TRUE);
...
-- Get investor portfolio for a specific investor
-- SELECT * FROM investor_portfolio WHERE investor_id = 3;

-- Get creator dashboard data
-- SELECT * FROM creator_dashboard WHERE creator_id = 1;

-- Get top performing movies by funding percentage
-- SELECT title, funding_percentage, investor_count FROM movie_investment_summary ORDER BY funding_percentage DESC LIMIT 10;